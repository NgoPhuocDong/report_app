...
import '../model/doanhthu_item.dart';
...
class DashboardPage extends StatelessWidget {
  ...
  const DashboardPage({Key? key, required this.model}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: model.fetchDoanhThuData(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          double maxY = model.doanhThuItems.isNotEmpty
              ? model.doanhThuItems.map((e) => e.value).reduce((a, b) => a > b ? a : b)
              : 0;
                  ...
                  child: Center(
                    child: Container(
                      ...
                      child: PageView.builder(
                        itemCount: (model.doanhThuItems.length / 5).ceil(),
                        itemBuilder: (context, pageIndex) {
                          int startIndex = pageIndex * 5;
                          int endIndex = (startIndex + 5 < model.doanhThuItems.length)
                              ? startIndex + 5
                              : model.doanhThuItems.length;
                          List<DoanhThuItem> pageItems = model.doanhThuItems.sublist(startIndex, endIndex);
  ...
  Widget _buildChart(List<DoanhThuItem> pageItems, double maxY) {
    return Container(
